# ZIRH-2 custom PDN configuration: byte-for-byte the LibreLane 3.0.5
# default (scripts/openroad/common/pdn_cfg.tcl) plus the macro connect
# rules marked 'ZIRH-2 deviation' below. Selected via PDN_CFG in
# src/config.json.
...
# --- ZIRH-2 deviation (do not remove) ----------------------------------------
# The chain-A macros (zirh_tmr_ff64) carry their power ring and pins on
# Metal4/Metal5: their metal stack is capped below the chip's TopMetal PDN
# so the stripes can cross the macro columns unbroken (TT precheck rejects
# split stripes). The default macro grid above only connects
# PDN_VERTICAL_LAYER<->PDN_HORIZONTAL_LAYER (TopMetal1<->TopMetal2), which
# never touches those pins - measured failure: PDN-0232 "does not contain
# any shapes or vias" x3, then PDN-0233. Connect the chip stripes down
# onto the macro power geometry:
add_pdn_connect \
    -grid macro \
